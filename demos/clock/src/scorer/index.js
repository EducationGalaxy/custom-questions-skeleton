export default class ClockScorer {
  ...
  isValid() {
    if (this.response) {
      ...
      const { valid_response, hour_tolerance, minute_tolerance } = question
      ...
      const lowestCorrectHour = correctHour - hour_tolerance
      const highestCorrectHour = correctHour + hour_tolerance
      const lowestCorrectMin = correctMin - minute_tolerance
      const highestCorrectMin = correctMin + minute_tolerance
      ...
      let minHandInRange =
        minHandAngle >= lowestCorrectMin && minHandAngle <= highestCorrectMin
      ...
      const handle360 =
        ...
        correctMin === 360
      if (handle0) {
        if (minHandAngle >= 360 - minute_tolerance) minHandInRange = true
        if (hourHandAngle >= 360 - hour_tolerance) hourHandInRange = true
      }
      if (handle360) {
        if (minHandAngle <= 0 + minute_tolerance) minHandInRange = true
        if (hourHandAngle <= 0 + hour_tolerance) hourHandInRange = true
      }
